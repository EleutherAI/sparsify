...
def triton_coo_sparse_dense_matmul(
    ...
    def grid(META):
        return triton.cdiv(AK, META["BLOCK_SIZE_AK"]), triton.cdiv(B, META["BLOCK_SIZE_B"])
...
def triton_sparse_transpose_dense_matmul_kernel(
    ...
    for ind in range(BLOCK_SIZE_AK):
        if ind + pid_ak * BLOCK_SIZE_AK < AK:
            ...
            if v != 0:
                accum += v * tl.load(
                    dense_ptr + a * stride_da + b_offset * stride_db, mask=mask_b,
                )
...
def triton_dense_dense_cooout_matmul_kernel(
    ...
    a_indices =  tl.load(
        at_indices_ptr + pid * BLOCK_SIZE_AK + offsets_ak, mask=pid * BLOCK_SIZE_AK + offsets_ak < AK
    )
    n_indices = tl.load(
        at_indices_ptr + pid * BLOCK_SIZE_AK + offsets_ak + AK, mask=pid * BLOCK_SIZE_AK + offsets_ak < AK
    )
    ...
    for k in range(K):
        ...
        dense1_chunk = tl.load(
            dense1_ptr + a_indices[:, None] * stride_d1a + b_offset[None, :] * stride_d1b,
            mask=dense1_mask
        ) * dense1_mask
        dense2_mask = (n_indices[:, None] < N) * (b_offset[None, :] < B)
        dense2_chunk = tl.load(
            dense2_ptr + n_indices[:, None] * stride_d2n + b_offset[None, :] * stride_d2b,
            mask=dense2_mask
        ) * dense2_mask
        ...
        accum += sums
    tl.store(out_ptr + pid * BLOCK_SIZE_AK + offsets_ak, accum, mask=pid * BLOCK_SIZE_AK + offsets_ak < AK)



class COODecoder(torch.autograd.Function):
    @staticmethod
    def forward(ctx, example_indices, feature_indices, feature_values, decoder_weight, N):
        ctx.save_for_backward(example_indices, feature_indices, feature_values, decoder_weight)
    ...
    def backward(ctx, grad_output):
        example_indices, feature_indices, feature_values, decoder_weight = ctx.saved_tensors
        ...
        feature_grad = dense_dense_cooout_matmul(
            grad_output,
            decoder_weight,
            torch.stack([example_indices, feature_indices])
        )
