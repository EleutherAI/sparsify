...
class FusedEncoder(torch.autograd.Function):
    ...
    def forward(
        ...
        if activation == "topk":
            ...
            was_padded = False
            if preacts.shape[1] % k != 0:
                prev_width = preacts.shape[1]
                preacts = F.pad(preacts, (0, k - preacts.shape[1] % k), value=-1000)
                was_padded = True
            ...
            if was_padded:
                indices_fit = indices < prev_width
                values = values.float() * indices_fit.float()
                indices = torch.where(indices_fit, indices, 0)
        else:
