...
def aggregate_optimizer_performance(df):
    ...
    min_fvu_summary = (
        fvu_df.groupby(['optimizer', 'skip', 'lr', 'layer'])['value']
        .min()
        .reset_index()
        .pivot(index=['optimizer', 'lr', 'layer'], columns='skip', values='value')
    ...
    win_summary = (
        ...
        .min()
        .reset_index()
        .pivot(index=['lr', 'layer', 'skip'], columns='optimizer', values='value')
        .reset_index()
    )

    adam_vals = win_summary.get('adam', np.inf)
    signum_vals = win_summary.get('signum', np.inf)

    win_summary['adam_win'] = adam_vals < signum_vals
    win_summary['signum_win'] = signum_vals < adam_vals

    win_counts = win_summary.groupby(['skip']).agg({
        'adam_win': 'sum',
        'signum_win': 'sum'
    }).reset_index()


    return min_fvu_summary.sort_values(['layer', 'lr', 'optimizer']), win_counts
...
def main(): 
    ...
    run_names = [
        f"SmolLM2-135M-signum-5e-3{momentum}",
        f"SmolLM2-135M-signum-5e-4{momentum}",
        f"SmolLM2-135M-signum-1e-4{momentum}",
        f"SmolLM2-135M-signum-1e-3{momentum}",

        f"SmolLM2-135M-skip-signum-5e-3{momentum}",
        f"SmolLM2-135M-skip-signum-5e-4{momentum}",
        f"SmolLM2-135M-skip-signum-1e-3{momentum}",
        f"SmolLM2-135M-skip-signum-1e-4{momentum}",
    ...
    optimizer_data_path = f"optimizer_data{momentum if momentum else '_m0.95'}.csv"
    if os.path.exists(optimizer_data_path):
        df = pd.read_csv(optimizer_data_path)
    else:
        df = load_wandb_data(run_names, metrics)
        df.to_csv(optimizer_data_path, index=False)

    print(momentum)
    optimizer_performance, win_counts = aggregate_optimizer_performance(df)
    print(win_counts)
    # print(optimizer_performance)
    # regular_df, skip_df = process_data(df, run_names)
    
    # fvu_fig = plot_metrics(regular_df, skip_df, "fvu", layer_nums)
    # dead_pct_fig = plot_metrics(regular_df, skip_df, "dead_pct", layer_nums)
    
    # fvu_fig.write_image(images_path / "fvu_metrics.pdf", format="pdf")
    # dead_pct_fig.write_image(images_path / "dead_neurons_metrics.pdf", format="pdf")

    # learning_rates = sorted(set(list(regular_df['lr'].unique()) + list(skip_df['lr'].unique())))
    # print(f"Found learning rates: {learning_rates}")
    
    # for lr in learning_rates:
    #     fvu_fig = plot_metrics_by_lr(regular_df, skip_df, "fvu", layer_nums, lr)
    #     dead_pct_fig = plot_metrics_by_lr(regular_df, skip_df, "dead_pct", layer_nums, lr)
        
    #     # Create clean filenames
    #     lr_str = lr.replace('-', '_')
    #     fvu_fig.write_image(images_path / f"fvu_metrics_lr_{lr_str}_m{momentum}.pdf", format="pdf")
    #     dead_pct_fig.write_image(images_path / f"dead_neurons_metrics_lr_{lr_str}_m{momentum}.pdf", format="pdf")
