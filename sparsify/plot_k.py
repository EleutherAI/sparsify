...
def load_wandb_data(run_names: list[str], metrics: list[str], project: str = "eleutherai/sparsify") -> pd.DataFrame:
    """
...
def main(): 
    ...
    skip_run_names = [
        ...
        # "SmolLM2-135M-skip-binary-topk-k=64-s=1" # can't find, queued on BYU wed night
        "SmolLM2-135M-topk-binary-s=2-k=64", 
    ...
    for df in [skip_df, sae_df]:
        if "k" not in df.columns:
            df['k'] = df['run_name'].str.split('-k=').str[1].str.split('_').str[0].str.split('-').str[0]
            df.loc[df['k'] == '', 'k'] = '32'
            df['k'] = df['k'].fillna(32)
    ...
    for k in [16, 32, 64, 128]:
        ...
        dead_pct_fig.write_image(images_path / f"btc_dead_neurons_metrics_k={k}.pdf", format="pdf")
    ...
    def concat_dfs(names, skip=False, activation='Binary'):
        ...
        for i, name in enumerate(names):
            ...
            # if k not in df columns, derive it from the file name (should have k=x), if not it's 32
            if 'k' not in df.columns:
                if "k=" in name:
            ...
            #Rename anyinstasnce of sparse_loss_increase to loss_increase, sparse_acc_decrease to acc_decrease etc.
            df.rename(columns={
    ...
    sae_binary_df = concat_dfs(sae_binary_evaluate_names, skip=False, activation='Binary')
    ...
    fig = make_subplots(
        ...
        cols=len(layer_nums), 
        shared_yaxes=True,            
        subplot_titles=[f"Layer {l}" for l in layer_nums],
    ...
    for col, layer in enumerate(layer_nums, 1):
        for src in [binary_df, topk_df]:
            df = src[src['hookpoint'] == f'layers.{layer}.mlp']
            activation = df['activation'].iloc[0]
        
            mean = df.groupby(['seed', 'k'])[['acc_decrease']].mean() # 'loss', 'loss_increase', 'acc', 'fvu'
            stds = df.groupby(['seed', 'k'])[['acc_decrease']].std() # 'loss', 'loss_increase', 'acc', 'fvu'

            mean['acc_decrease_std'] = stds['acc_decrease']
            mean = mean.reset_index()

            stats = agg_stats(df)       # plot_df from binary runs
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    name=activation,
                    showlegend=col == 1,
                    line=dict(color=colors[activation]),
                    error_y=dict(type='data',
                                array=stats['sem'],
                                arrayminus=stats['sem'],
                                visible=True)),
                row=1,
                col=col,
            )
    ...
    fig.write_image(images_path / "btc_acc_decrease_by_k.pdf", format="pdf")

    fig = make_subplots(
        rows=1, cols=len(layer_nums), shared_yaxes=True,
        subplot_titles=[f"Layer {l}" for l in layer_nums],
        horizontal_spacing=0.02,
        vertical_spacing=0.08,
                    )
    ...
    for col, layer in enumerate(layer_nums, 1):
        for src in [sae_topk_df, sae_binary_df]:
            df = src[src['hookpoint'] == f'layers.{layer}.mlp']
            activation = df['activation'].iloc[0]
    ...
    fig.write_image(images_path / "bae_acc_decrease_by_k.pdf", format="pdf")
